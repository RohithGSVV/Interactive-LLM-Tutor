...
class ChromaClient():
    ...
    def retrieve_top_matches(host: str, port: str, collection_name: str, query: str, 
                            n_results: int = 10, ssl: bool = False, auth_token: str = None) -> List[Dict]:
        protocol = "https" if ssl else "http"
        url = f"{protocol}://{host}:{port}"

        try:
            client = chromadb.HttpClient(host=host, port=port, ssl=ssl)  # , auth_token=auth_token)
            print(f"Successfully connected to ChromaDB at {url}")

            # Create an embedding function (use the same one as in upload_documents)
            embedding_function = embedding_functions.DefaultEmbeddingFunction()
...
def exec_upload_documents(args):
    ...
    if args.documents and args.metadatas and args.ids:
        upload_success = upload_documents(
            host=args.host,
            port=args.port,
            collection_name=args.collection,
            documents=args.documents,
